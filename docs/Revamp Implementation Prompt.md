...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Web hosting   | Next.js SSR on **Vercel**                                                                                                                               | Next.js (`output: "standalone"`) container on **ECS Fargate + CloudFront**, 100% Terraform-managed (Amplify Hosting / OpenNext-on-Lambda are managed alternatives) |
...
- **Web app**: **migrated off Vercel to AWS** — the Next.js app runs as a containerized ECS Fargate service behind CloudFront (managed alternatives: AWS Amplify Hosting or OpenNext-on-Lambda). It only uploads to S3, creates/reads jobs, and renders persistent progress.
...
- **Database access**: a typed query layer / migration tool (Prisma). All schema changes are versioned SQL migrations checked into the repo. Do not mutate schema by hand.
...
- **Suggested repository layout** (monorepo; adapt to conventions but keep the separation):
  ```text
  apps/web/                 # existing Next.js app (upload + status UI only for media); containerized for ECS Fargate + CloudFront
...
- **Modules to author** (one per bounded component):
  ...
  - `iam` — **separate least-privilege task roles** for the web app, API, FFmpeg, transcription, and scoring workers (plan §11).
  ...
  - `web` — frontend delivery: an ECS Fargate service for the containerized Next.js app (on the shared ECS cluster) plus a CloudFront distribution + WAF for TLS, static/edge caching, and edge protection. (Swap for an `aws_amplify_app` if the managed Amplify option is chosen.)
...
- Deploy the existing Next.js web app to AWS — containerized on ECS Fargate behind CloudFront — and stop deploying it to Vercel. The app only does direct-to-S3 uploads plus job status/polling UI.
...
- Terraform for: network, storage (S3), database (RDS), queue (SQS, DLQ from day one), compute (API + one FFmpeg worker + integration workers), **web (Next.js on ECS Fargate + CloudFront)**, ECR, IAM roles, Secrets Manager/KMS, baseline CloudWatch.
...
- The web app is served from AWS (CloudFront + ECS Fargate), not Vercel; the Vercel deployment is retired for the migrated path.
...
- [ ] Web app served entirely from AWS (Next.js on ECS Fargate + CloudFront, Terraform-managed); Vercel project decommissioned after cutover.
- [ ] Legacy browser-FFmpeg, Vercel hosting, and Vercel-Blob storage / synchronous paths retired after cutover.
