...
export async function POST(request: Request) {
  ...
  return Sentry.withServerActionInstrumentation("POST /api/process", { recordResponse: true }, async () => {
    try {
      ...
      Sentry.setTag("locale", outputLanguage);
      ...
      Sentry.setContext("process_input", {
      ...
      // Fetch audio from Vercel Blob URL (bypasses the 4.5 MB serverless payload limit)
      const { audioBuffer, audioParseTime, audioSizeMB } = await Sentry.startSpan(
        { name: "blob.fetch_audio", op: "http.client" },
        async () => {
          const t0 = Date.now();
          const blobResult = await blobGet(audioUrl, { access: "private" });
          if (!blobResult || !blobResult.stream) {
            throw new Error("Failed to fetch audio from storage");
          }
          const buf = Buffer.from(await new Response(blobResult.stream).arrayBuffer());
          const elapsed = Date.now() - t0;
          const sizeMB = (buf.length / 1024 / 1024).toFixed(2);
          return { audioBuffer: buf, audioParseTime: elapsed, audioSizeMB: sizeMB };
        }
      );
      console.log(`[API] Audio fetched from blob: ${audioSizeMB}MB (fetch: ${audioParseTime}ms)`);
      ...
      const { segments, transcriptionTime } = await Sentry.startSpan(
        { name: "elevenlabs.transcribe", op: "ai.pipeline" },
        async (span) => {
          const t0 = Date.now();
          const segs = await transcribeAudioFromBuffer(audioBuffer);
          const elapsed = Date.now() - t0;
          span.setAttribute("audio_size_mb", parseFloat(audioSizeMB));
          span.setAttribute("segment_count", segs.length);
          return { segments: segs, transcriptionTime: elapsed };
        }
      );
      ...
      const { clips: clipCandidates, tokenUsage, geminiTime } = await Sentry.startSpan(
        { name: "gemini.generate_clips", op: "ai.pipeline" },
        async (span) => {
          const t0 = Date.now();
          const result = await generateClipCandidates(segments, mergedPreferences, outputLanguage);
          const elapsed = Date.now() - t0;
          span.setAttribute("clip_count", result.clips.length);
          if (result.tokenUsage) {
            span.setAttribute("tokens_input", result.tokenUsage.tokens_input);
            span.setAttribute("tokens_output", result.tokenUsage.tokens_output);
          }
          return { ...result, geminiTime: elapsed };
        }
      );
      ...
      Sentry.captureException(error, {
        tags: { request_id: requestId, route: "/api/process" },
        extra: { message },
      });
